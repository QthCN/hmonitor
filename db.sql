...
create table TRIGGER_EVENTS(id int not null AUTO_INCREMENT, trigger_name varchar(256) not null, hostname varchar(256) not null, event varchar(256) not null, value varchar(256) not null, last_occur_time timestamp not null, first_occur_time timestamp not null, severity varchar(256) not null, occur_amount int not null, status varchar(128) not null, primary key (id), key(trigger_name), key(last_occur_time), key(first_occur_time), key(status), key(severity));
